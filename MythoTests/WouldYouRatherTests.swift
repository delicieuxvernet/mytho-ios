...
final class WouldYouRatherTests: XCTestCase {
    ...
    func testTheDeckHoldsTheBaseDilemmas() {
        XCTAssertEqual(WouldYouRatherBank.all.count, 10)
    }
    ...
    func testTheExtremePackStaysBehindTheAgeGate() {
        XCTAssertEqual(WouldYouRatherBank.all.count, 10)
        XCTAssertEqual(WouldYouRatherBank.extreme.count, 14)

        XCTAssertEqual(WouldYouRatherBank.dilemmas(adultUnlocked: false, extremeEnabled: true).count, 10,
                       "Sans confirmation d'âge, l'interrupteur seul ne suffit pas")
        XCTAssertEqual(WouldYouRatherBank.dilemmas(adultUnlocked: true, extremeEnabled: false).count, 10,
                       "L'âge confirmé n'active rien tant que la table n'a pas choisi")
        XCTAssertEqual(WouldYouRatherBank.dilemmas(adultUnlocked: true, extremeEnabled: true).count, 24)

        let ids = Set(WouldYouRatherBank.extreme.map(\.id))
        XCTAssertEqual(ids.count, 14, "Identifiants uniques")
